...
while True:
	...
	inB=GPIO.input(sensor1)
	if inB == 1 and estado2 == 0:
		estado1=1-estado1
		time.sleep(0.1)
...
while True:
	...
	estado2=inB
        ...
        if estado1 == 1:
                GPIO.output(sensor2,1)
        else:
                GPIO.output(sensor2,0)
...
class myHandler(BaseHTTPRequestHandler):
	...
	def do_GET(self):
		...
		try:
			...
			sendReply = False
			if self.path.endswith("/"): #si el string termina con html eso significa endswith
				mimetype='text/html'
